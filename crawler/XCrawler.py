...
# Regex to match date in 'until' and 'since' parameters.
DATE_SINCE_UNTIL = r'^(?!0000)[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$'

# Vertical size of the scroll to scroll down the page
Y = 500

# Regex to match the video preview link
# ACTUAL_VIDEO_PREVIEW_PATTERN = '^https:\/\/pbs\.twimg\.com\/ext_tw_video_thumb.*' # only video, not including gif
ACTUAL_VIDEO_PREVIEW_PATTERN = '^https:\/\/pbs\.twimg\.com\/*' 
...
class XCrawler:
    ...
    def complete_search(self):
        ...
        while True:
            ...
            for raw_tweet in self.raw_tweets:
                username_tweet_id = raw_tweet.find('a', {'class':"css-1rynq56 r-bcqeeo r-qvutc0 r-37j5jr r-a023e6 r-rjixqe r-16dba41 r-xoduu5 r-1q142lx r-1w6e6rj r-9aw3ui r-3s2u2q r-1loqt21"})                
    ...
    def compute_scroll_pause_time(self):
        lower_bound = round(self.wait_scroll_base - self.wait_scroll_epsilon, 2) 
        upper_bound = round(self.wait_scroll_base + self.wait_scroll_epsilon, 2) 
    ...
    def get_results(self):
        if self.mode == 0:
            return self.get_actual_images(), self.get_video_preview()
        else:
            return self.tweets
